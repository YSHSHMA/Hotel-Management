...
if (missingRequiredEnvVars.length > 0) {
  ...
  missingRequiredEnvVars.forEach((envVar) => console.error(`   - ${envVar}`));
  process.exit(1);
}
...
const cloudinaryConfigPresent =
  process.env.CLOUDINARY_CLOUD_NAME &&
  process.env.CLOUDINARY_API_KEY &&
  process.env.CLOUDINARY_API_SECRET;

if (cloudinaryConfigPresent) {
  cloudinary.config({
  ...
  console.warn(
    "⚠️ Cloudinary is not configured (CLOUDINARY_* env vars missing). Image upload features may fail."
  );
}
...
const connectDB = async () => {
  ...
  // Validate scheme early for clearer errors
const startsWith = (prefix: string) => mongoUri.trim().toLowerCase().startsWith(prefix);
  const isValidScheme =
    startsWith("mongodb://") ||
  ...
  if (!isValidScheme) {
    ...
    // Print a safe hint (don’t log full credentials)
    const shown = trimmed.length > 80 ? trimmed.slice(0, 80) + "..." : trimmed;
    console.error("❌ Invalid MONGODB_CONNECTION_STRING scheme.");
    console.error(
      "Expected it to start with \"mongodb://\" or \"mongodb+srv://\""
    );
    console.error(`Detected value preview: ${shown}`);
    process.exit(1);
  }
...
app.get("/", (req: Request, res: Response) => {
  res.send(`
    <h1>Backend is active</h1>
    <p>For Swagger documentation, visit:</p>
